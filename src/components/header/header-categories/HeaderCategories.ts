...
export class HeaderCategories extends HTMLElement {
  domChanger: HeaderCategoriesDomHandler[] = [
    new HeaderCategoriesDomString(".js--title", "title", this),
    new HeaderCategoriesDomLinkHref(".js--link", "link", this),
    new HeaderCategoriesDomPosts(".js--posts", "posts", this),
  ];
  ...
  registerButtons() {
    const buttons: NodeListOf<HTMLElement> = this.querySelectorAll(
      ".js--category-button"
    );
  ...
  extractButtonData(button: HTMLElement): HeaderCategoriesData {
    ...
    const missingKeys = Object.keys(data).filter(
      (key) => !data[key as keyof HeaderCategoriesData]
    );
